using DanilvarKanji.Application.CharacterLearnings;
...
public class CharacterLearningController : ApiController
{
    ...
    public async Task<IActionResult> CreateAsync([FromBody] CreateCharacterLearningRequest request)
    {
        ...
        var command = new CreateCharacterLearningCommand(
            user!,
            request.CharacterId,
    ...
    [ProducesResponseType(typeof(IEnumerable<CharacterLearning>), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    public async Task<IActionResult> ListCompletelyLearnedAsync(
        [FromQuery] PaginationParams paginationParams
    )
    {
        AppUser? user = await _userManager.GetUserAsync(User);
    ...
    [ProducesResponseType(
        typeof(IEnumerable<CharacterLearningResponseBase>),
        StatusCodes.Status200OK
    )]
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    public async Task<IActionResult> ListReviewQueueAsync(
        [FromQuery] PaginationParams paginationParams
    )
    {
        AppUser? user = await _userManager.GetUserAsync(User);

        var characters = await Mediator.Send(new ListCurrentReviewQuery(paginationParams, user!));
    ...
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    public async Task<IActionResult> ListFutureReviewQueueAsync(
        [FromQuery] PaginationParams paginationParams
    )
    {
        AppUser? user = await _userManager.GetUserAsync(User);

        var characters = await Mediator.Send(new ListFutureReviewQuery(paginationParams, user!));
    ...
    public async Task<IActionResult> GetNextInReviewQueueAsync()
    {
        ...
        CharacterLearningResponseBase? learning = await Mediator.Send(
            new GetNextToReviewInQueueQuery(user!)
        );
    ...
    public async Task<IActionResult> GetRandomMeaningsInReviewAsync(
        string characterId,
    ...
    )
    {
        ...
        RandomItemsInReview? result = await Mediator.Send(
            new GetRandomMeaningsInReviewQuery(characterId, user!, culture, qty)
        );

        if (
            string.IsNullOrEmpty(result?.CorrectMeaning)
            || result.RandomItems == null
            || !result.RandomItems.Any()
        )
    ...
    [ProducesResponseType(StatusCodes.Status204NoContent)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<IActionResult> GetRandomKunReadingsInReviewAsync(
        string characterId,
        int qty = 4
    )
    {
        ...
        var (random, correct) = await Mediator.Send(
            new GetRandomKunReadingsInReviewQuery(characterId, user!, qty)
        );
    ...
    public async Task<IActionResult> GetRandomOnReadingsInReviewAsync(string characterId, int qty = 4)
    {
        ...
        var (random, correct) = await Mediator.Send(
            new GetRandomOnReadingsInReviewQuery(characterId, user!, qty)
        );
