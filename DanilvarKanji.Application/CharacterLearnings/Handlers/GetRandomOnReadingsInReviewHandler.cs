...
public class GetRandomOnReadingsInReviewHandler
  : IRequestHandler<GetRandomOnReadingsInReviewQuery, (List<string> random, string correct)>
{
  private readonly ICharacterRepository _characterRepository;
  private readonly ICharacterLearningRepository _charLearningRepository;
  private readonly Random _random;

  public GetRandomOnReadingsInReviewHandler(
  ...
  public async Task<(List<string> random, string correct)> Handle(
    GetRandomOnReadingsInReviewQuery request,
    CancellationToken cancellationToken
  )
  {
    var randomKunyomiFromCharacter = _characterRepository.GetRandomOnyomiFromCharacter(
      request.CharacterId
    );
  ...
  {
    var charLearningsToLearn = await _characterRepository.GetRandomOnReadingsInLearnQueueAsync(
      request.AppUser,
      characterQty
    );
