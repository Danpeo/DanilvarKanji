...
public class ExtractTextFromImageHandler : IRequestHandler<ExtractTextFromImageCommand, Result<string>>
{
    ...
    public async Task<Result<string>> Handle(ExtractTextFromImageCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            string text = Extractor.ExtractTextFromMemoryStream(ms, _settings.TessdataPath, request.LangMode);
