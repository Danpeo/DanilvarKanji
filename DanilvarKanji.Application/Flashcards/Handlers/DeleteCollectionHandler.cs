...
public class DeleteCollectionHandler : IRequestHandler<DeleteCollectionCommand, Result<string>>
{
    ...
    public DeleteCollectionHandler(IFlashcardRepository flashcardRepository, IUnitOfWork unitOfWork,
    ...
    public async Task<Result<string>> Handle(DeleteCollectionCommand request, CancellationToken cancellationToken)
    {
        if (await _flashcardRepository.ExistAsync(request.CollectionId, request.AppUser))
        {
            ...
            {
                _logger.LogInformation("DELETED Collection: {@collection} for {@user}", 
                    request.CollectionId, request.AppUser);
                
                return Result.Success(request.CollectionId);
            }
        }

        _logger.LogError("FAILED to DELETE Collection: {@collection} for {@user}", 
            request.CollectionId, request.AppUser);
