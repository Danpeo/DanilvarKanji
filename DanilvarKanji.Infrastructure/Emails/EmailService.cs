...
public class EmailService : IEmailService
{
    ...
    public async Task CheckConfirmationCodeAsync(
        string email,
    ...
    {
        var expectedCode = await _userRepository.GetRegistrationConfirmationCodeAsync(email);

        if (expectedCode != null)
        {
            var confirmed = expectedCode == confirmationCode;
            if (confirmed)
