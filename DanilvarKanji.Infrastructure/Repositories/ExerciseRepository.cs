...
public class ExerciseRepository : IExerciseRepository
{
  ...
  {
    var exercises = await GetExsWithRelatedData()
  ...
  {
    return await GetExsWithRelatedData().FirstOrDefaultAsync(x => x.Id == id && x.AppUser == user);
  }

  public Task<bool> AnyExist()
  {
    return _context.Exercises.AnyAsync();
  }

  public Task<bool> Exist(string id, AppUser user)
  {
    return _context.Exercises.AnyAsync(x => x.Id == id && x.AppUser == user);
  }

  public Task<bool> AnyToReview(AppUser appUser)
  {
    return _context.Exercises.AnyAsync(x => x.AppUser == appUser);
  }

  private IQueryable<Exercise> GetExsWithRelatedData()
  {
    var exs = _context.Exercises.AsSplitQuery().Include(x => x.Character).Include(x => x.AppUser);
